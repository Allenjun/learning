...
public class UserProvider implements ProviderMethodResolver {
    ...
    public static String insertOneAnno(@Param("user") UserDO user) {
        SQL sql = new SQL();
        sql.INSERT_INTO("t_user").VALUES("name, password, sex", "#{user.name}, #{user.password}, #{user.sex}");
        return sql.toString();
    }
    ...
    public static String insertAnno(@Param("users") List<UserDO> users) {
        ...
        String join = String.join(",", users.stream()
            .map(user -> "(" + (Objects.isNull(user.getName()) ? "null"
                : "'" + user.getName() + "'") + "," + (Objects.isNull(user.getPassword()) ? "null"
                : "'" + user.getPassword() + "'") + ", " + (Objects.isNull(user.getSex()) ? Sex.MALE.ordinal()
                : user.getSex().ordinal()) + ")")

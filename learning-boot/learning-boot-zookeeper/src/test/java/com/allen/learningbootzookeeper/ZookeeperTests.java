...
public class ZookeeperTests {
    ...
    /**
     * @description:
     *      命名服务，在指定路径创建服务名作为Znode的节点名,值为服务地址
     */
    @Test
    @SneakyThrows
    public void testNamingService() {
        ...
        serviceNamingMap.forEach((k, v) -> {
            try {
                client.create(k, v.getBytes(StandardCharsets.UTF_8),
    ...
    /**
     * @description:
     *      配置中心，利用zookeeper维护一个分成命名空间，Znode的值就是配置数据
     */
    @Test
    @SneakyThrows
    ...
    /**
     * @description:
     *      服务注册与发现，服务提供者在/providers目录下创建自己的url的节点；
     *      服务消费者在/consumers目录下创建自己的url的节点，并订阅/providers目录
     ...
     * @description:
     *      分布式锁：
     *      1. 独占锁：创建/删除临时节点，实现锁的获取和释放
     *      2. 顺序锁：创建顺序临时节点，监听自己的节点是否是最小的节点，是则获得锁；不是就监听前一个节点的删除操作
     */
    @Test
    @SneakyThrows
    ...
    public void testCreate() {
        ...
        client.create(persistent_sequential, new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT_SEQUENTIAL);
        client.create(ephemeral_sequential, new byte[0], Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
    }
    ...
    public void testGet() {
        ...
        client.getData(persistent, event -> {

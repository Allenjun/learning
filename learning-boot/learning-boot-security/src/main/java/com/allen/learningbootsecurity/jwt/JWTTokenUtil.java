...
public final class JWTTokenUtil {
    ...
    private static byte[] DEFAULT_SECRET = "123456789@ABCDEFGHIJKLMNOPQRSTUVWXYZ".getBytes(DEFAULT_CHARSET);
    ...
    public static void main(String[] args) {
        String jws = Jwts.builder().setSubject("Joe").setExpiration(DateUtils.addSeconds(new Date(), expire))
    ...
    public static String generateToken(Payload payload) {
        return Jwts.builder().setSubject(payload.getSub()).setExpiration(DateUtils.addSeconds(new Date(), expire))
            .signWith(key).compact();
    }

...
import com.allen.learningbootsecurity.security.MyAccessDeniedHandler;
import com.allen.learningbootsecurity.security.MyAuthenticationEntryPoint;
import com.allen.learningbootsecurity.security.MyAuthenticationSuccessHandler;
import com.allen.learningbootsecurity.security.MyCorsConfigurationSource;
import com.allen.learningbootsecurity.security.MyCsrfRequestMatcher;
import com.allen.learningbootsecurity.security.MySessionInformationExpiredStrategy;
...
//        configureJWT(http);
    ...
    private void configureInMemory(AuthenticationManagerBuilder auth) throws Exception {
        auth
            .inMemoryAuthentication()   // 实际注册了InMemoryUserDetailsManager
            .passwordEncoder(bCryptPasswordEncoder())
            .withUser("admin").password(bCryptPasswordEncoder().encode("admin")).roles("ADMIN", "NORMAL").and()
            .withUser("allen").password(bCryptPasswordEncoder().encode("allen")).authorities("ROLE_NORMAL").and();
    }
...
//            .authenticationEntryPoint(myAuthenticationEntryPoint);
        ...
        http.csrf()
            .csrfTokenRepository(
                new LazyCsrfTokenRepository(new HttpSessionCsrfTokenRepository()))     // token仓库，负责生成、获取、保存token
...
//        http.portMapper().http(8888).mapsTo(443);
        ...
        http.authorizeRequests()
            .antMatchers("/dynamicPerm").access("@rbacService.hasPermission(request, authentication)");
        
        /* Basic登录 (注册：BasicAuthenticationFilter) */
        http.httpBasic()
            .authenticationEntryPoint(myAuthenticationEntryPoint);   // 配置认证失败时处理逻辑
...
//            .failureHandler(new SimpleUrlAuthenticationFailureHandler())
            ...
            .withObjectPostProcessor(new ObjectPostProcessor<UsernamePasswordAuthenticationFilter>() {
                ...
                public <O extends UsernamePasswordAuthenticationFilter> O postProcess(O object) {
                    object.setRequiresAuthenticationRequestMatcher(new AntPathRequestMatcher("/login", "POST"));
                    return object;
                }
